...
Панель часто тримають доступною з інтернету, тому варто розуміти, що
відкрито, а що ні.

**Потребує входу адміністратора:** керування сервісами, редактор конфігів,
менеджер оновлень, керування статичними TG BrandMeister, зміна пароля.

**Доступно без входу:** головна сторінка, системні метрики, стрічка
активності, мережевий статус, останні 30 рядків логів сервісів зі списку.
Тобто позивний, DMR ID, підключені мережі й хто виходив в ефір видно всім,
хто відкриє сторінку.

**Захист входу:**

- прогресивна затримка на невдалу спробу: 5, 10, 20, 30, 60 секунд
- після 5 невдалих спроб IP блокується на годину
- спроби пишуться в journal: `journalctl -u dashboard | grep auth`
- при вході показується список невдалих спроб з IP і часом

> Блокування живе в пам'яті процесу — `systemctl restart dashboard` його
> скидає. Це навмисно: власник машини завжди може розблокувати себе,
> а зловмисник доступу до `systemctl` не має.

**Що зробити після встановлення:**

1. Змінити типовий пароль (`passw0rd`) на довгий і випадковий — захист від
   перебору сильно ускладнює атаку, але не рятує від словникового пароля
2. Якщо панель не потрібна назовні — не переадресовувати порт `85` з роутера,
   а ходити через VPN
3. Не публікувати архів конфігів: у ньому `admin_pass.hash` і `bm_apikey`
...
Повноекранний монітор ефіру: хто зараз в ефірі з прапорцем країни,
статус усіх мереж, карта географії активності за добу.

![Live-сторінка з картою активності](docs/screenshots/14.png)

Стрічка подій з позивними і прапорцями. Країна, з якої йде передача,
підсвічується червоним; перемикачі над картою фільтрують її за режимом.

![Стрічка подій і карта](docs/screenshots/15.png)
